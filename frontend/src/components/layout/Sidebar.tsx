...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, theme, onLogout }) => {
  return (
    <aside className={`fixed left-0 top-0 h-full w-80 z-40 hidden lg:block border-r transition-all ${
      theme === 'dark' ? 'bg-slate-900/90 border-slate-800 backdrop-blur-3xl' : 'bg-white border-slate-200 shadow-lg'
    }`}>
      <div className="p-8">
        <div className="flex items-center gap-4 mb-12 group">
          ...
          <div>
            <h1 className={`text-2xl font-black tracking-tighter font-display ${
              ...
              Sheep<br /><span className="text-cyan-400">Master</span>
            </h1>
        ...
        <nav className="space-y-3">
          ...
          <div className="pt-6 mt-6 border-t border-slate-800">
             ...
             <button onClick={onLogout} className="w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-rose-400 hover:bg-rose-950/40 hover:text-rose-300 transition-all mt-4 group border border-transparent hover:border-rose-800/40">
