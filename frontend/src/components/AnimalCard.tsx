  ...
  return (
    ...
    `}>
      ...
      <div className="p-6 pt-4 flex flex-wrap gap-2">
        ...
        {onFicha && (animal.especie || 'Ovino') === 'Ovino' && (
          <button
            onClick={(e) => { e.stopPropagation(); onFicha(); }}
            title="Ficha genealógica en PDF"
            className={`group/btn flex-1 flex flex-col items-center gap-1.5 p-3 rounded-2xl transition-all border ${
              isDark
                ? 'bg-slate-800/80 border-slate-700/60 hover:bg-amber-600/30 hover:border-amber-500 text-slate-200'
          ...
          >
            ...
            <span className="text-[9px] font-black uppercase">Ficha</span>
          </button>
        )}
      </div>
    </div>
